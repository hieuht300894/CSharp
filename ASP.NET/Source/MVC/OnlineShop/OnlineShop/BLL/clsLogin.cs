...
namespace OnlineShop.BLL
{
    public static class clsLogin
    {
        ...
        {
            xAccount account = instance.GetRepository<xAccount>().GetAll().FirstOrDefault(x => x.Username.ToLower().Equals(username.ToLower()) && x.Password.ToLower().Equals(password.ToLower()));
        ...
        {
            xAccount account = instance.GetRepository<xAccount>().GetAll().FirstOrDefault(x => x.Username.ToLower().Equals(username.ToLower()));
